...
export const useCommitFi = (): UseCommitFiReturn => {
  ...
  const createChallenge = useCallback(async (
    ...
    try {
      ...
      // For existing apps, we need to create a new instance
      ...
      // Deploy new app instance with create_challenge
      const { appClient } = await factory.deploy({
        onSchemaBreak: algokit.OnSchemaBreak.AppendApp,
        onUpdate: algokit.OnUpdate.AppendApp,
        createParams: {
  ...
  const verifyParticipant = useCallback(async (
    ...
    try {
      ...
      await client.send.call.verifyParticipant({
        args: { participant, isValid }
      })
  ...
  const distributePayout = useCallback(async (appId: bigint) => {
    ...
    try {
      ...
      await client.send.call.distributePayout()
